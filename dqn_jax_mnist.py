...
"""
    ...
    def update(q_state, observations, actions, next_observations, rewards, dones):
        observations = observations.reshape(128,-1)
        next_observations = next_observations.reshape(128,-1)
    ...
    for global_step in range(args.total_timesteps):
        ...
        max_q_value = q_values[actions]

        if random.random() < epsilon:
            actions = np.array(random.randrange(10))
            q_diff = max_q_value - q_values[actions]
            exploration_moves += 1
        else:
