...
"""
    ...
    if args.track:
        ...
        config = wandb.config
        for key in ['env_id', 'sigma', 'sigma_n', 'posterior_update_freq', 'posterior_batch_size', 'w_sample_freq',
                    'alpha', 'seed']:
    ...
    q_state = TrainState.create(
        ...
        params=feature_network.init(q_key, obs),
        ...
        target_params=feature_network.init(q_key, obs),
    ...
    state_coverage_dict = defaultdict(int)
    rewards_sum = 0
    episodes = 0
    exploration_moves = 0
    ...
    for global_step in range(args.total_timesteps):
        ...
        phi = feature_network.apply(q_state.params, obs)
        ...
        q_diff = 0
        expected_q_diff = expected_q_values[0][expected_actions[0]] - expected_q_values[0][actions[0]]

        if actions != expected_actions:
            q_diff = q_values[0][actions[0]] - expected_q_values[0][expected_actions[0]]
            exploration_moves += 1

        state = np.array([np.round(obs[0][0]), np.round(obs[0][1]), 1 if obs[0][2] > 0 else 0, np.round(obs[0][3])])
        key = (tuple(state), tuple(actions))
        state_coverage_dict[key] += 1
        ...
        if global_step % 100 == 0:
            ...
            writer.add_scalar("exploration/expected_q_diff", np.array(expected_q_diff).item(), global_step)
            q_variance = np.var(q_values)
            writer.add_scalar("exploration/q_variance", q_variance.item(), global_step)
        ...
        if "final_info" in infos:
            for info in infos["final_info"]:
                if info and "episode" in info:
                    ...
                    writer.add_scalar("exploration/cumulative_states_covered", len(state_coverage_dict), global_step)
                    writer.add_scalar("exploration/exploration_moves", exploration_moves, global_step)

                    rewards_sum += info["episode"]["r"][0]
                    episodes += 1
        ...
        if global_step > args.learning_starts:
            ...
            if global_step % args.posterior_update_freq == 0:
                ...
                targets = compute_targets(q_state,
                                          ...
                                          data.rewards.flatten().numpy(),
                                          data.dones.flatten().numpy())
                phi = feature_network.apply(q_state.params, data.observations.numpy())
